...
export default function bestMatchMap<T>(
  ...
  const mapGrid = pairMapGrid.reduceRight((mapGrid: MatchMap[][], row, aIndex) => {
    const mapRow = row.reduceRight((mapRow: MatchMap[], myMap, bIndex) => {
      let bestMatch = myMap;
      const rightNeighbor = mapRow[bIndex + 1];
      if (rightNeighbor && rightNeighbor.score > bestMatch.score) bestMatch = rightNeighbor;
      const downNeigbor = mapGrid[aIndex + 1] && mapGrid[aIndex + 1][bIndex];
      if (downNeigbor && downNeigbor.score > bestMatch.score) bestMatch = downNeigbor;
      const diagNeigbor = mapGrid[aIndex + 1] && mapGrid[aIndex + 1][bIndex + 1];
