...
describe("isWithCode", () => {
  ...
  test("returns false when type guard fails", () => {
    const partial = { code: "en", name: 123 };
    expect(isWithCode(partial, isLanguage)).toBe(false);
  });
});
...
describe("lessonProgress", () => {
  ...
  test("returns 0 when matching lesson has falsy progress", () => {
...
describe("calcLessonProgress", () => {
  ...
  test("filters by motherTongue strings when motherTongue=true", () => {
    ...
    // Only the motherTongue string is considered, and it is translated
    expect(result.progress).toBe(100);
  });

  test("returns 0 when motherTongue=true but no motherTongue strings", () => {
    const lStr = makeLessonString({ motherTongue: false });
    const result = calcLessonProgress(true, [lStr], []);
    expect(result.progress).toBe(0);
  });
});
