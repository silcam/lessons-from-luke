...
export default function srcCompare(aLang: CompLang, bLang: CompLang): Comparison {
  ...
  const lessonWeight = percentSame(aLessons.length, bLessons.length) / lessonCount;
  let totalWeight = 0.0;
  ...
  for (let lessonIndex = 0; lessonIndex < lessonCount; ++lessonIndex) {
    ...
    totalWeight += lessonWeight * percentSame(aLessonMTStrings, bLessonMTStrings);
  }
