...
describe("srcCompare", () => {
  test("returns 100% with no errors for identical languages", () => {
    const lesson: SrcStrings = [makeLesson([{ text: "Hello", mtString: true }])[0]];
  ...
  test("reports MT string count mismatch within a lesson", () => {
    ...
    const lessonB = makeLesson([{ text: "Hello", mtString: true }]);
    const result = srcCompare(
  ...
  test("handles empty lessons arrays", () => {
    const result = srcCompare({ name: "Lang A", lessons: [] }, { name: "Lang B", lessons: [] });
    expect(result.errors).toHaveLength(0);
  });
});
