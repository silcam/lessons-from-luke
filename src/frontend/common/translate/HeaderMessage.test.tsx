...
describe("HeaderMessage", () => {
  it("renders nothing when hdrMessage is 'none'", () => {
    const { container } = renderWithProviders(<HeaderMessage hdrMessage="none" />, {
  ...
  it("renders 'unsavedChanges' message", () => {
    const { getByText } = renderWithProviders(<HeaderMessage hdrMessage="unsavedChanges" />, {
      syncState: defaultSyncState,
    });
  ...
  it("renders 'changesSaved' message when not uploading", () => {
    const { getByText } = renderWithProviders(<HeaderMessage hdrMessage="changesSaved" />, {
      syncState: {
        ...defaultSyncState,
        connected: true,
