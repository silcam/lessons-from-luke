/**
 * UsersPage.test.tsx — unit tests for the standalone admin Users page with
 * the Revoke device access control
 *
...
describe("UsersPage — users list", () => {
  it("shows a Revoke device access button for each user after loading", async () => {
    const { findAllByText } = renderWithProviders(<UsersPage />, {
...
describe("UsersPage — Revoke confirm dialog", () => {
  it("opens a confirm dialog when Revoke device access is clicked", async () => {
    const { findAllByText, findByRole } = renderWithProviders(<UsersPage />, {
      syncState: defaultSyncState,
      currentUser: adminUser,
    });
  ...
  it("dialog has a Revoke button and a Cancel button", async () => {
    const { findAllByText, findByRole, getByRole } = renderWithProviders(<UsersPage />, {
      syncState: defaultSyncState,
      currentUser: adminUser,
    });
  ...
  it("closes dialog on Cancel without calling the API", async () => {
    const { findAllByText, findByRole, queryByRole } = renderWithProviders(<UsersPage />, {
      syncState: defaultSyncState,
      currentUser: adminUser,
    });
  ...
  it("announces success via aria-live region after revoke", async () => {
    ...
    const { findAllByText, findByRole, findByText } = renderWithProviders(<UsersPage />, {
      syncState: defaultSyncState,
      currentUser: adminUser,
    });
