/**
 * UsersPage.tsx — Admin management screen: list all users + revoke device access
 *
 * Spec: specs/004-desktop-auth-pairing/spec.md §FR-017
 * Plan: specs/004-desktop-auth-pairing/plan.md §Presentation Design (UI
 *       Decisions: Web admin Revoke device access), §Accessibility Requirements
 *
 * Renders:
 *   - Loading indicator while fetching; a distinct error state with retry
 *   - List of all users with a destructive "Revoke device access" button per
 *     row. Click opens an accessible confirm dialog (native <dialog> with
 *     showModal for focus trapping). On confirm: POST revoke-sessions.
...
export default function UsersPage() {
  ...
  return (
    <>
      <StdHeaderBarPage
        title={t("AdminHome_users_heading")}
