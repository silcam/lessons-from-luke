/**
 * gateway.ts — bulk unfiltered row-fetch I/O layer for the `restoreLesson`
 ...
 *
 * Every function here issues raw SQL against a `PGStorage`/`PGSnapshotStorage`
 * instance's `sql` tag — NOT the `Persistence` interface's `languages()` /
 * `tStrings()` methods, because those filter out archived languages and
 * legacy `lessonStringId`-scoped rows, which is exactly what diagnosis must
 * see. Each function is a thin, individually unit-testable wrapper
 * (fixture-driven, per research D11) returning typed rows. None of these
 * functions filter archived languages or legacy `lessonStringId` rows when
 * called the way diagnose/apply/verify call them (unrestricted). They are
 * reused identically by `diagnose` (baseline), `apply`'s I11 drift re-check,
 * and `verify`'s I19 sweep — do not duplicate this SQL at those call sites.
 */
import { SqlFunc } from "postgres";
import { Language } from "../../../core/models/Language";
import { TString } from "../../../core/models/TString";
import { BaseLesson, Book } from "../../../core/models/Lesson";
import { DuplicateRow, LessonRef } from "./types";
 ...
 */
export async function fetchTStringsForLesson(
  sql: SqlFunc,
  lessonId: number,
  masterIds: number[],
  ...
  const byLegacyLessonStringIdRows: TString[] = await sql`
    SELECT masterid, languageid, sourcelanguageid, source, text, history, lessonstringid
    FROM tstrings
 ...
 */
export async function fetchLegacyScopedCount(sql: SqlFunc): Promise<number> {
...
export async function fetchLessonByBookSeriesLesson(
  sql: SqlFunc,
  book: Book,
  series: number,
  lesson: number
): Promise<BaseLesson | null> {
  const rows: BaseLesson[] = await sql`
 ...
 */
export async function fetchDuplicateRowSweep(
  sql: SqlFunc,
  masterIds: number[]
 ...
 * under diagnosis themselves; this wrapper does no filtering.
 */
export async function fetchLessonsSharingMasterIds(
  sql: SqlFunc,
  masterIds: number[]
): Promise<MasterIdLessons[]> {
  if (masterIds.length === 0) return [];
  const rows = await sql`
