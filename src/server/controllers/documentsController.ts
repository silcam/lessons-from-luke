...
import { uploadEnglishDoc, uploadNonenglishDoc } from "../actions/uploadDocument";
import bodyParser from "body-parser";
import { DocUploadMeta, isEnglishUpload } from "../../core/models/DocUploadMeta";
...
export default function documentsController(app: Express, storage: Persistence) {
  app.get("/api/languages/:languageId/lessons/:lessonId/document", async (req, res) => {
    handleErrors(res, async () => {
      ...
      if (isNaN(majorityLangId))
        majorityLangId = language.motherTongue ? language.defaultSrcLang : language.languageId;

      const filepath = await makeLessonFile(storage, lesson, language, majorityLangId);
      res.sendFile(filepath);
    });
  });

  app.post("/api/admin/documents", formDataParser, fileUpload(), async (req, res) => {
    handleErrors(res, async () => {
