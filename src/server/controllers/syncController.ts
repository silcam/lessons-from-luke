...
export default function syncController(app: Express, storage: Persistence) {
  addGetHandler(app, "/api/sync/:timestamp/languages/:languageTimestamps?", async (req) => {
    const languageTimestamps = decodeLanguageTimestamps(req.params.languageTimestamps || "");
