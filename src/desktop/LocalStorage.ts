...
import { StoredSyncState, initalStoredSyncState } from "../core/models/SyncState";
import { PublicLanguage, LessonProgress, calcLessonProgress } from "../core/models/Language";
import { TString, equal } from "../core/models/TString";
import { app } from "electron";
import fs from "fs";
import path from "path";
import { modelListMerge } from "../core/util/arrayUtils";
import { OnSyncStateChangePayload, ON_SYNC_STATE_CHANGE } from "../core/api/IpcChannels";
...
export default class LocalStorage {
  ...
  constructor(basePath?: string) {
    ...
    if (this.memoryStore.localStorageVersion < LOCAL_STORAGE_VERSION) this.migrateLocalStorage();
  ...
  recalcProgress(app: DesktopApp | null) {
    ...
    const progress: LessonProgress[] = lessons.map((lesson) =>
      calcLessonProgress(language.motherTongue, this.getLessonStrings(lesson.lessonId), tStrings)
    );
    this.setSyncState({ language: { ...this.getSyncState().language!, progress } }, app);
  }
  ...
  setProjectLanguageTStrings(tStrings: TString[]) {
    ...
    if (!languageId) throw "setProjectLanguageTStrings called without set Project Language!";

    const existing = this.getAllTStrings(languageId);
    const newTStrings = tStrings.map((tStr) => {
      const oldTStr = existing.find((oldTStr) => oldTStr.masterId == tStr.masterId);
      return oldTStr ? { ...tStr, history: oldTStr.history.concat([oldTStr.text]) } : tStr;
    });
    return this.setTStrings(languageId, newTStrings);
  }

  setDocPreview(lessonId: number, preview: string) {
    this.writeTextFile(docPreviewFilename(lessonId), desktopifyDocPreview(preview));
  }
  ...
  protected logFileName(logType: LogType, date: Date = new Date(), opts: { daily?: boolean } = {}) {
    let name = `LOG-${logType}-${date.getUTCFullYear()}-${zeroPad(date.getUTCMonth() + 1, 2)}`;
    if (opts.daily) name += "-" + zeroPad(date.getUTCDate(), 2);
    return name;
  }

  protected manageLogs() {
    const logFileNames = fs.readdirSync(this.basePath).filter((name) => name.startsWith("LOG"));
    ...
    logFileNames.forEach((filename) => {
      if (filename.startsWith("LOG-DataUsage-") && filename < `LOG-DataUsage-${today}`) {
    ...
    toDelete.forEach((filename) => unlinkSafe(path.join(this.basePath, filename)));
  }
  ...
  protected readFile<T>(filename: string, defaultValue: T): T {
    try {
      ...
      this.writeLogEntry("Error", `[LocalStorage.readFile : ${filename}] ${err}`);
      throw err;
    }
  }
