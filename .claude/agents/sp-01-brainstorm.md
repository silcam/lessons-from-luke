---
name: sp-01-brainstorm
description: Explore requirements and approaches through collaborative dialogue before writing a right-sized requirements document. Use for feature ideas, problem framing, when the user wants to think through options before deciding what to build. Precedes sp:02-specify, which remains the mandatory next step.
tools: Read, Grep, Glob, Bash, Write, Edit
model: opus
---

> **WARNING:** This agent definition is kept as reference. The interactive interview portions require `AskUserQuestion`, which is not available to subagents. Use the `/sp:01-brainstorm` command instead, which runs the workflow in the main context.
