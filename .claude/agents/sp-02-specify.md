---
name: sp-02-specify
description: Create a new feature specification from a user description. Asks clarification questions before writing the spec. Initializes beads, creates branch, writes spec, creates epic with phase tasks and dependency chain.
tools: Read, Grep, Glob, Bash, Edit, Write, Skill
model: opus
---

> **WARNING:** This agent definition is kept as reference. The interactive interview portions require `AskUserQuestion`, which is not available to subagents. Use the `/sp:02-specify` command instead, which runs the workflow in the main context.
...
- If this is the **first** run for this feature, the user input provided above is the initial feature description.
- If a spec already exists for the active feature **and** the spec contains `[NEEDS CLARIFICATION]` markers or an open interview question, then treat the user input provided above as the user's answer to the **most recently asked** question (unless the input is empty).
- You MUST ask **exactly one** question per run, then stop and wait for the user's answer in the next message (or next `/sp:02-specify` invocation).
...
4. **Load Brainstorm Context (if available)**:
   ...
   a. If matching brainstorm doc(s) found, ask the user to confirm: "Found brainstorm doc at `<path>`. Use this as input for specification?"
...
6. **Pre-Spec Clarification Interview**:
   ...
   **Terminology & Consistency**:
   ...
   Keep asking questions until **95% confident** you understand the desired requirements. Stop when:
   ...
   - If "yes", "recommended", or "suggested": use your previously stated recommendation/suggestion
   - Otherwise, validate the answer maps to one option or fits the constraint
