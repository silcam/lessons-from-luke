import fs from "fs";
import translateFromUsfm from "../../src/core/translateFromUsfm";
...
test("USFM Translate: A few valid refs", () => {
  const result = translateFromUsfm(sampleTStrings, sampleUSFM).translations;
...
test("USFM translate Book Mismatch", () => {
  const actsTStrings = [
    {
      ...sampleTStrings[1],
      src: sampleTStrings[1].src.replace("Luc", "Actes")
    }
  ];
  expect(
    translateFromUsfm(actsTStrings, sampleUSFM).translations["0"]
  ).toBeUndefined();
});
...
test("USFM translate Other Books/Languages", () => {
  ...
  expect(translateFromUsfm(lukeTStrings, noHdrUsfm).translations["3"]).toEqual(
  ...
  expect(translateFromUsfm(actesTStrings, actsUSFM).translations["1"]).toEqual(
  ...
  expect(translateFromUsfm(actsTStrings, actsUSFM).translations["3"]).toEqual(
...
test("USFM Translator only translates MTStrings", () => {
  ...
  expect(
    translateFromUsfm(notMTStrings, sampleUSFM).translations["0"]
  ).toBeUndefined();
});
...
test("Multiword MT Book Name", () => {
  ...
  expect(result.translations["1"]).toContain("2 Luka");
});
