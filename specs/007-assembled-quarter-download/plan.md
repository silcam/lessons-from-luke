...
- **[MEDIUM-HIGH] Result retention via `docStorage` 24 h cleanup is silently broken unless the merged output is written through `tmpFilePath()`.** `cleanTmpDir()` prunes by `parseInt(filename) < old` — it only ever deletes files whose on-disk name **begins with the epoch-ms timestamp** that `tmpFilePath()` prepends (`${timestamp}_${baseName}`). If the assembled output is written with a human-facing name like `English_Luke-Q1-bilingual.odt`, `parseInt(...)` is `NaN`, `NaN < old` is `false`, and the file is **never pruned** → unbounded disk leak, defeating the plan's "24 h cleanup reused for result retention" claim. Mitigation: the assembled `.odt` (and every intermediate flattened constituent) MUST be created via `docStorage.tmpFilePath(...)` so it carries the timestamp prefix; the **on-disk** name is decoupled from the `Content-Disposition` download filename (the latter is set per `documentName()` conventions at stream time, contract §4). Do not hand-name files into `docsTmpPath()`.
