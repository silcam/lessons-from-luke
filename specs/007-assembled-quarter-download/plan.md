...
## Adversarial Hardening (Red-Team Pass 3)

Pass 3 surfaced **one new HIGH** — an implicit single-process assumption underneath the whole in-memory job model that both prior passes took for granted — plus a **MEDIUM second-order effect of Pass 2's mitigation** and one LOW tidy. Because a new HIGH appeared, convergence-by-severity was **not** reached this pass; the `/sp:04-red-team` loop terminates here only because outer iteration 3 is the max. Access control remains out of scope; neither finding adds authorization.

### Deployment Topology (new — undermines the core job-model guarantees)

- **[HIGH] The in-memory registry + concurrency-1 gate silently break under multi-worker Passenger; the single-process assumption MUST be made explicit and enforced.** The entire design (registry dedup, serialized soffice, queue-depth cap, poll-by-key) assumes **one** Node process. Production runs under Passenger (Technical Context), and there is **no** `passenger_max_pool_size` / `min_instances` pin anywhere in `config/` — Passenger's default pool scales to multiple app worker processes on demand. Under >1 worker, each has its **own** `AssemblyJobRegistry`, so:
  - **(a) FR-010 dedup breaks.** Two operators (or a double-click) routed to different workers each see "no live job" and both run a full ~40 s assembly for the same key — the exact redundant work FR-010 forbids.
  - **(b) concurrency-1 breaks as a resource serializer.** The per-job isolated `-env:UserInstallation` profile (Pass 1) prevents mutual _corruption_ across concurrent soffice runs, but does **not** prevent N workers from running N merges **simultaneously** — the CPU/RAM/disk pressure the concurrency-1 gate exists to bound. Both halves must be stated: no corruption, but no serialization either.
  - **(c) the queue-depth cap becomes per-worker** → effective live-job ceiling is N× the intended cap, defeating Pass 1's resource bound.
  - **(d) poll-by-key can land on a worker that never held the job** → spurious `404` → the client re-requests (contract §2/§4), which under (a) can spin up yet another redundant assembly, compounding.
  - **Codebase evidence this is a regression, not a theoretical worry:** every other piece of shared server state already lives in Postgres _precisely_ to survive multiple workers — `getAuthPool()` / better-auth sessions replaced the old `cookie-session` for exactly this reason. This feature would be the **first** server state whose correctness depends on process affinity.
  - **Mitigation (in-scope — spec Out-of-Scope bars durable/queued infra, so no DB/queue):** make the assumption explicit and enforced, one of:
    (i) **pin Passenger to a single app process** (`passenger_max_pool_size 1`, and set `passenger_min_instances 1`) and record it as a **required deploy constraint** — note this enforcement lives in deploy/web-server config (Capistrano / Passenger), **not** application code, so it must be tracked as an operational task, not a code change; or
    (ii) explicitly **accept the degradation** under the "small-team, low-concurrency" premise and record FR-010 + concurrency-1 as **best-effort under multi-worker**, with the isolated-profile decoupling (Pass 1) as the only cross-process safety net.
    The plan takes **(i)**: pin to one process so dedup and serialization hold as designed. Design-impacting → registry process-scoping assumption recorded in `data-model.md`; the per-process nature of the queue-depth cap noted in `contracts/assembly-api.md` §1. No new endpoint or schema.

### Temp-Dir Lifecycle (second-order effect of Pass 2 finding B)

- **[MEDIUM] Per-job working dirs orphan on abrupt process death — routine on every deploy — with no app-controlled cleanup path.** Pass 2 finding B deliberately moved the constituents + flattened copies into a per-job `mktemp` working dir **outside** `docStorage` so they'd skip the 24 h `cleanTmpDir` sweep and avoid the flat-dir collision. The side effect: their **only** cleanup path is the eager `rm -rf` on completion/caught-crash. On **abrupt** death — SIGKILL / OOM / and critically `custom:restart_passenger`, which runs on **every** Capistrano deploy (`config/deploy.rb`) — that `rm -rf` never executes and the working dir survives. There is **no app-controlled cleanup path** for these; the design would be relying on OS tmp policy, which is not guaranteed (and doesn't apply at all if the working root is app-local rather than under the OS temp dir). Over many deploys the residue accumulates. Mitigation: place per-job working dirs under a **single known dedicated root** (e.g. `<docStorage>/assembly-work/<jobId>/`), still isolated per job (preserving Pass 2's collision fix), and **sweep that root on server startup** (registry init) so orphans from a prior process are reclaimed on the next boot — which a deploy triggers anyway. A bare `mktemp -d` in the default temp location cannot be swept selectively and won't survive as a reclaimable unit. Design-impacting → job working-dir lifecycle added to `data-model.md`.

### Consistency Tidy

- **[LOW] State that completeness/generation runs in the `running` phase, not in the accept handler.** The contract (§1 Note) permits completeness "synchronously at start **or** reported as a `failed` job on first poll," but `data-model.md` defines completeness as "each of the 14 generates via `makeLessonFile` without error" — the expensive generation step. Running that synchronously inside the `POST` accept handler would contradict FR-007's whole reason for a background job. Clarification (no behavior change): the cheap **existence** check (TOC + 13 expected numbers present in `storage.lessons()`) MAY run synchronously at accept to return an immediate block; the **generation** half of completeness always runs in the `running` phase. Logic-only — no contract/data-model schema impact.

## Adversarial Hardening (Red-Team Pass 4)

Pass 4 surfaced **one genuinely new MEDIUM** the prior three passes missed: a state-changing-POST CSRF surface that diverges from an existing, codebase-mandated security convention. Severity continues to converge downward (Pass 1 multiple Highs → Pass 2 one High → Pass 3 one High → Pass 4 one Medium). No new Critical/High. Access control remains out of scope; this finding adds **no** authorization rule — it hardens the new POST's origin surface, exactly as the codebase already does for its other state-changing POSTs.

### Cross-Site Request Forgery (new — diverges from an established codebase convention)

- **[MEDIUM] The new state-changing `POST …/assembly` MUST enforce the same `requireSameOrigin` CSRF check the codebase already mandates for state-changing POSTs.** This feature adds the first state-changing `POST` outside the `/api/auth/*` surface (`POST /api/languages/:languageId/quarters/:book/:series/assembly`). It kicks off expensive resource-consuming work (14× `makeLessonFile` + a ~40 s `soffice` merge). The codebase already established — through the invitation feature's own red-team passes — that state-changing POSTs get the `src/server/middle/requireSameOrigin.ts` middleware (Origin/Referer validated against the `trustedOrigins` allow-list, `403` otherwise; see `invitationController.ts`, which applies it to `POST /api/admin/invitations` and `POST /api/auth/invitation/accept` and covers it with live integration tests). Better-auth's own `formCsrfMiddleware` protects only `/api/auth/*`, **not** custom controllers, so the assembly POST would ship with **no** origin check — a divergence from the codebase's own security bar that `/sp:08-harden`'s security-review would flag later anyway. **Framing (in scope):** `requireSameOrigin` gates _how_ (cross-origin) a request arrives, not _who_ may access — it protects the existing access from cross-origin abuse, so it introduces no new access rule (consistent with the spec's "no new access rules of its own" and with how prior passes kept access control out of scope). **Bounded-impact caveat (why MEDIUM, not HIGH):** better-auth's session cookie defaults to `SameSite=Lax` (confirmed against `node_modules/better-auth`), which already prevents a cross-site POST from carrying the session cookie in current browsers; the attacker also cannot read the returned `jobId` (opaque cross-origin response), and the Pass 1 queue-depth cap (`429`) bounds resource amplification. So this is primarily **defense-in-depth consistency** with a control the codebase treats as required — hence MEDIUM on bounded impact, but non-negotiable on precedent. **Mitigation:** apply `requireSameOrigin` to the assembly `POST` route only. The read-only **GET** status/download endpoints are exempt (mirrors `invitationController`'s explicit "no CSRF check on read-only GET" note — a cross-origin caller cannot read the JSON/blob body anyway). **No frontend change:** browsers attach the `Origin` header automatically on same-origin XHR, so `useAssembleQuarter` needs no change. Design-impacting → `contracts/assembly-api.md` §1 gains a `403 Forbidden` (foreign/missing Origin) response. No data-model impact (controller middleware, not job-model/schema).

## Adversarial Hardening (Red-Team Pass 5)

Pass 5 surfaced **one genuinely new CRITICAL** — a data-loss path rooted in how the _reused-unchanged_ `makeLessonFile` actually returns its output, which every prior pass took for granted — plus one **MEDIUM** gap in the job-model's attach semantics that silently breaks FR-009 retry. Because a new CRITICAL appeared, convergence-by-severity is **not** reached this pass. Access control remains out of scope; neither finding adds authorization.

### Constituent Provenance — Source-Document Corruption (new — CRITICAL)

- **[CRITICAL] The pre-process/flatten step MUST copy each constituent into the per-job working dir and operate on the copy — it MUST NEVER mutate a `makeLessonFile`-returned path in place, because that path can be a canonical source document.** Two verified facts about the reused-unchanged pipeline drive this:
  1. `makeLessonFile` (`src/server/actions/makeLessonFile.ts:15`) **short-circuits the English+English case and returns `docStorage.docFilepath(lesson)` — the raw admin-uploaded source `.odt` under `docs/`** — not a tmp copy. This fires precisely for **English mother-tongue bilingual assembly**, which is exactly the golden-reference case (`English_Luke-Q<n>-Master-bilingual.odt`) the plan's own mandated `*.integration.test.ts` must assemble to assert FR-003/FR-004 parity. So the dangerous path is **exercised by the acceptance harness**, not hypothetical.
  2. For every other case, `makeLessonFile` (`makeLessonFile.ts:34`) hardcodes its output to `docStorage.tmpFilePath(`${motherLang.name}_${lessonName(lesson)}.odt`)` in the **flat `docStorage` dir** — the caller cannot redirect it to the per-job `assembly-work/<jobId>/` dir.
  - **Consequence if flatten runs in place:** for English bilingual, `flattenFooterFields` (a `styles.xml`/zip mutation, plan "Security & Robustness") or the eager `rm -rf`/cleanup would **modify or delete the canonical source ODT in `docs/`**. In production `docs/` holds admin-uploaded English **source** documents that are non-recoverable without re-upload — this is destruction of canonical source data, hence **CRITICAL**, not merely a robustness nit.
  - **Mitigation (in-scope; reconciles "reuse `makeLessonFile` unchanged" with the Pass 2/3 per-job-dir design):** the assembly action MUST **copy** each `makeLessonFile` result into `assembly-work/<jobId>/` and run `flattenFooterFields` + the merge against the **copy**, treating the `makeLessonFile`-returned path as strictly read-only. This copy is also the _only_ mechanism that gets constituents into the per-job working dir at all — `makeLessonFile` physically cannot target it — so it is what makes Pass 2 finding B (collision-safe intermediates) and Pass 3's startup-sweep working-dir design actually achievable, rather than aspirational. Name the copies with **ASCII, deterministic, insertion-order** filenames (`00.odt` = TOC, `01.odt`..`13.odt`), which additionally closes the macro surface: the injected StarBasic `insertDocumentFromURL(...)` then references only controlled ASCII `file://` URLs, never an arbitrary Unicode DB-stored language name interpolated into a StarBasic string literal / URL (a language name with `"`, `\`, a space, `&`, or non-Latin script would otherwise break the macro or the URL). The human-facing, language-named download filename is set separately at stream time (contract §4), unaffected.
  - **Note (do not reopen Pass 2's collision rationale):** because generation runs _inside_ the serialized `running` slot (concurrency-1 covers generate+merge, per Pass 2 finding D's "run-start = slot acquisition"), the copy step reintroduces no cross-job collision; the per-job dir stands on profile-isolation + startup-sweep grounds independently. This is an addition, not a churn of prior findings.
  - Design-impacting → constituent provenance + copy-before-flatten + ASCII deterministic naming recorded in `data-model.md` (working-dir lifecycle + validation rules). No contract change.

### Job-Model Attach Semantics — Retry Path (new — MEDIUM)

- **[MEDIUM] `startOrAttach` behavior on a key whose existing job is _terminal_ (`failed`/`ready`) is unspecified, and the wrong default silently breaks FR-009 retry.** The data-model attach rule is defined only for a `queued`/`running` ("live") job ("start on a key with an existing queued/running job returns that job"). It says nothing about a `POST` arriving for a key whose only entry is terminal. FR-009 requires the operator to **re-trigger to retry** after a failure, and the "result availability after completion" edge case lets the operator re-request after a `ready` result was pruned. If `startOrAttach` blindly attached to _any_ existing entry, a `POST` over a `failed` job would return the stale failure forever with **no way to retry** — a direct FR-009 violation.
  - **Mitigation:** make the terminal-attach rule explicit. **Non-negotiable:** a `POST` over a `failed` entry (and over a `ready` entry whose `resultPath` is already gone) MUST **evict the terminal entry and start a fresh job** (new `jobId`, `queued`). **Judgment call to state, not silently pick:** a `POST` over a still-valid `ready` entry (file present) MAY either return the existing `ready` job (client re-downloads — cheaper, no freshness requirement in spec) **or** rebuild; the plan chooses **return-existing-if-valid, rebuild-if-pruned-or-failed** (reuse when possible, since there is no freshness bar). Dedup/attach (FR-010) therefore applies to `queued`/`running`/valid-`ready`; `failed` and pruned-`ready` are treated as absent. Design-impacting → contract §1 (attach-vs-restart on terminal) and `data-model.md` attach rule.
