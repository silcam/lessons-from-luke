...
- **The concurrency-1 slot is a resource owned by the _runner_, not a property of job _status_.** It is acquired on promotion and released only when the runner's promise actually settles. A job's status may go terminal long before that, and the two are deliberately decoupled: marking a job `failed` is a statement to the **user**, while holding the slot is a statement about a **process** that may still be alive. `evict()` therefore does not free the slot either (an evicted job's runner keeps it until it settles), and the release is **identity-guarded** so a late settlement can never free a successor's slot.
- Enforces a per-job hard timeout → `failed`. **The registry timeout does _not_ kill soffice** — `sofficeAssemble`'s own timer kills the process group. The timeout marks the job `failed` and aborts the runner's `AbortSignal` (threaded through `assembleQuarter` into `sofficeAssemble`), but it **does not free the slot or promote a successor**, because the runner may still own a live soffice — that is precisely how two merges end up side by side. It MUST still be ordered strictly after soffice's own self-kill so a merely-slow job is never failed out from under a soffice that was about to succeed: `ASSEMBLY_TIMEOUT_MS = DEFAULT_TIMEOUT_MS (180 s) + ASSEMBLY_NON_SOFFICE_BUDGET_MS (120 s)` = 300 s (`assemblyBudget.ts`; see research.md §R5). **Terminal statuses are immutable**: a runner that settles after its job already timed out has its _outcome_ ignored (with a `console.warn`), so a `failed` job is never resurrected as `ready` and its TTL is never extended — but that settlement **does** release the slot, which is the very event the slot was being held for. **The timeout clock starts at run-start (slot acquisition), not at enqueue (Pass 2 finding D):** with concurrency-1 plus the queue-depth cap, jobs can legitimately sit `queued` behind ~40 s of prior work; timing from enqueue would spuriously `fail` a job before it ever ran. Queued wait is bounded by the queue-depth cap (admission control), not by timing out already-admitted jobs.
- **`ASSEMBLY_ABANDON_MS` escape hatch.** If a runner never settles at all, holding the slot would wedge the feature for the life of the process. The runner's only unbounded awaits are DB queries — `PGStorage` configures no `statement_timeout`/`query_timeout`/`connect_timeout`, and an `AbortSignal` cannot cancel an in-flight porsager query — so a slot held past `ASSEMBLY_TIMEOUT_MS + ASSEMBLY_NON_SOFFICE_BUDGET_MS` is force-released and `console.error`'d. Evaluated lazily from `startOrAttach`/`get`/`getByKey`, matching the registry's no-background-timer design. The residual risk is acceptable because of _which_ scenario reaches it: soffice self-kills at `DEFAULT_TIMEOUT_MS`, so reaching the registry timeout at all means the **non-soffice** work overran, where there is provably no live soffice to collide with.
- **Observable consequence:** a retry (or a distinct-key successor) after a timeout reports `queued` until the orphaned runner settles, rather than starting immediately. The frontend treats `queued` and `running` identically, so the UI is unchanged.
...
- Owns per-job working-dir + profile lifecycle. **Working dirs live under a single known dedicated root, not a bare `mktemp -d`, and are swept on startup (Pass 3 finding — MEDIUM):** each job's constituents, flattened copies, and soffice profile live under `<docStorage>/assembly-work/<jobId>/` (still isolated per job — preserves Pass 2 finding B's collision fix). They are `rm -rf`'d eagerly on completion/caught-crash **and** the whole `assembly-work/` root is swept on registry init at server startup, because the eager cleanup never runs on abrupt death (SIGKILL / OOM / `custom:restart_passenger`, which fires on **every** Capistrano deploy). Without the startup sweep these orphans have **no app-controlled cleanup path** (they sit outside the `docStorage` 24 h `cleanTmpDir` sweep by Pass 2's design, and OS tmp policy is not guaranteed / does not apply to an app-local root). **Orphaned _processes_ are reaped before the `rm -rf`** (`reapOrphanedSoffice`): soffice is spawned `detached`, so it survives the restart that stranded its dir, and no in-process abort channel can reach across a restart — leaving it alive both deletes its profile tree out from under it and puts a second merge alongside the fresh registry's free concurrency-1 slot. Identity is established by scanning `/proc` for a LibreOffice command line carrying our own per-job `-env:UserInstallation=…/<jobId>/…` argument (not a recorded pid — pid reuse, and `oosplash` forks `soffice.bin`, so the spawned pid may not be the live one), and the kill targets the process group read from `/proc/<pid>/stat`. An unreadable `/proc` (macOS dev, a container without it) makes the whole reaper **inert**, mirroring `availableSystemMemory`'s precedent. Like the sweep itself, this is safe only under the single-process deployment constraint above.
