...
- Enforces a per-job hard timeout → `failed`. **The registry timeout does _not_ kill soffice** — the registry has no abort channel; `sofficeAssemble`'s own timer kills the process group. The registry's only power is to mark the job `failed` and free the concurrency-1 slot, so it MUST be ordered strictly after soffice's own self-kill: `ASSEMBLY_TIMEOUT_MS = DEFAULT_TIMEOUT_MS (180 s) + ASSEMBLY_NON_SOFFICE_BUDGET_MS (120 s)` = 300 s (`assemblyBudget.ts`; see research.md §R5). Firing first would promote a queued job alongside a still-live soffice. **Terminal statuses are immutable** as a backstop: a runner that settles after its job already timed out is ignored (with a `console.warn`), so a `failed` job is never resurrected as `ready`, its TTL is never extended, and a successor's slot is never clobbered. **The timeout clock starts at run-start (slot acquisition), not at enqueue (Pass 2 finding D):** with concurrency-1 plus the queue-depth cap, jobs can legitimately sit `queued` behind ~40 s of prior work; timing from enqueue would spuriously `fail` a job before it ever ran. Queued wait is bounded by the queue-depth cap (admission control), not by timing out already-admitted jobs.
