...
- **FR-002**: The assembled document MUST be self-contained and fully editable when opened: no protected/locked sections and no externally linked content. _(Contingent on the WS-2a assembly feasibility spike — see Assumptions.)_
- **FR-003**: Page numbering in the assembled document MUST run continuously across all lessons, and each lesson's first page MUST suppress its page number, matching the output the manual master-document workflow produces today. _(Contingent on the WS-2a assembly feasibility spike — see Assumptions.)_
...
- **Spike gate (blocking for planning).** Requirements FR-002 and FR-003 — "fully editable" **and** "continuous numbering with first-page suppression" together — are contingent on the WS-2a assembly feasibility spike proving the assembly approach can deliver both. The spike has **not** yet been performed. This product spec is written technology-agnostically and is stable regardless of the assembly mechanism chosen, but `/sp:03-plan` MUST NOT proceed until the spike confirms the approach (or an alternative is chosen and this spec revisited). A spike task is created as a blocker on the plan phase.
