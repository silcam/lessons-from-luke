...
- **FR-003**: **⚠️ SUPERSEDED 2026-08-11 by client feedback — see `specs/brainstorms/2026-08-11-quarter-pagination-and-coloring-page-style-requirements.md`.** The client reviewed a delivered quarter and reported the page numbering as a defect. Two of this requirement's positions are reversed: the page-number offset is no longer "expected" (the `-1`/`-2` `text:page-adjust` values in the template assets are to be removed, with each numbering sequence pinned explicitly instead), and the first lesson MUST begin on a recto page (per-lesson recto starts remain out of scope). The new scheme is roman front matter restarting at arabic `1` on lesson 1, rather than one continuous sequence matched to the reference deliverable. Read the rest of this requirement as historical context for why the offsets exist, not as current doctrine. Original text follows.

  The assembled document's pagination and layout MUST match Chris's reference deliverable for that quarter — the `English_Luke-Q<n>-Master-bilingual.odt` files (see Assumptions, "Reference deliverables"). Concretely, this means: page numbering runs continuously across all lessons (never resetting per lesson), each lesson's first page suppresses its page number, and the page-number sequence matches the reference. The reference is the oracle; the assembled output is not required to be "book-correct" beyond matching it. Explicitly **out of scope**: forcing each lesson to begin on an odd (right-hand) page — the reference does not do so, and this MUST is dropped. The reference carries a page-number = physical-page + 1 offset; because the reference is the target, that offset is expected, not a defect, and no offset-removal work is required. _(WS-2a spike confirmed continuous numbering + first-page suppression are feasible and already match the reference's offset; the only pagination-adjacent gap is FR-004's blank footer fields. See Assumptions.)_
...
- **Spike gate (RESOLVED — GO).** The WS-2a assembly feasibility spike is complete (`spike/FINDINGS.md`; beads `lessons-from-luke-koog.1`). Result: **GO.** LibreOffice headless (`soffice`) can merge the TOC + 13 lesson ODTs into one fully-editable `.odt` (0 protected/linked sections — FR-002 confirmed) with continuous page numbering and per-lesson first-page suppression (FR-003 confirmed), preserving all image references. Mechanism: `insertDocumentFromURL` driven from a StarBasic macro in-process in `soffice` (LO's bundled Python is unusable on macOS but fine on the Linux server; either driver reaches the same UNO call). **Gaps documented by the spike:** (a) pagination carries a page-number = physical + 1 offset and lessons don't start on odd/right-hand pages — **both since resolved as non-issues** by the decision to match Chris's reference deliverable, which carries the identical offset and likewise starts no lesson on an odd recto (verified 2026-07-08 by rendering the reference `.odt` to PDF; page number = physical + 1 throughout, matching A2's output). No offset-removal or odd-page work is in scope. (b) the per-lesson footer **Quarter/Lesson number fields render blank** after merge (they are `meta.xml` custom-property-backed `text:user-defined` fields that `insertDocumentFromURL` does not carry over). This must be closed to satisfy FR-004's "output consistent with the per-lesson download"; the spike documents a scriptable fix (flatten each lesson's footer fields to literal text before insertion). `/sp:03-plan` may now proceed and MUST budget that field-flattening step. Concurrency note: `soffice` is effectively single-instance — serialize the assembly step or use isolated per-job profiles.
- **Second spike gate — `.odm` master-document route (RESOLVED — NO-GO).** A sibling feasibility spike (`spike/odm/FINDINGS-odm.md`) tested approach **A1** (Chris's manual `.odm` master-document workflow, scripted) because it was believed to fix the two A2 pagination gaps. It does **not**. Validated against ground truth: A2's output, a scripted `.odm`, and **Chris's actual `English_Luke-Q2-Master-bilingual.odm`** (fetched from Drive, rewired to these Luke-2 inputs) **all render the same +1 page-number offset and none start lessons on odd rectos**. Chris's master carries no `text:page-adjust`/offset field, so the offset is **not** a property the `.odm` route provides — it is inherent to the shipped output. Because the feature now targets parity with that shipped output (FR-003), this offset is expected and requires no fix. **Decision: the assembly mechanism is A2** (`insertDocumentFromURL`); `/sp:03-plan` proceeds on A2. One A1-only lead for the footer gap: Chris's **protected** linked sections preserve per-lesson footer numbers where A2 blanks them (a possible alternative to the field-flatten fix — verify in planning).
- **Reference deliverables (acceptance oracle).** Chris's assembled per-quarter deliverables — `English_Luke-Q<n>-Master-bilingual.odt` for Q1–Q4, plus their `-TOC.odt` and per-lesson `-L##.odt` inputs (provided under `Downloads/lessons-from-luke/Luke bilingual editable files/English Teacher_s Guides/`) — are the ground-truth target for a correct assembly. "Match the reference" means **functional/visual equivalence**, not a byte-for-byte ODT diff (two production routes — LibreOffice merge vs. Chris's manual export — never produce clean file diffs; incidental metadata, style names, and ordering differ). The axes checked are: (1) content and lesson ordering (TOC + lessons 1–13); (2) continuous page numbering that never resets per lesson; (3) each lesson's first page suppresses its number; (4) footer Quarter/Lesson fields populated (the reference's populated fields are the oracle for FR-004's footer gap — verified present in the reference); (5) the page-number sequence matches the reference (including its +1 offset). One or more reference `.odt` files should be committed as test fixtures / golden references during planning.
