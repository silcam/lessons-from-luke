...
- `403 Forbidden` — `{ "error": "Forbidden" }`
  When the request's `Origin` (or, failing that, `Referer`) header is missing or not in the `trustedOrigins` allow-list. This is a **state-changing POST**, so it MUST run the codebase's existing `requireSameOrigin` CSRF middleware (`src/server/middle/requireSameOrigin.ts`) — the same control applied to the other state-changing POSTs (`POST /api/admin/invitations`, `POST /api/auth/invitation/accept`). This adds **no** authorization rule (it validates request origin, not identity) and needs **no** frontend change (browsers send `Origin` automatically on same-origin XHR). See plan "Adversarial Hardening (Red-Team Pass 4)". The read-only GET endpoints in §2–§4 are exempt (a cross-origin caller cannot read their body).
