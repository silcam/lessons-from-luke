...
- **Timeout + kill**: **two** timers, and their ordering is an invariant. `sofficeAssemble`'s own `DEFAULT_TIMEOUT_MS` (**180 s**) is the only one that **kills** the soffice process group (a hung soffice sits at 0 % CPU forever). The registry's per-job `ASSEMBLY_TIMEOUT_MS` merely marks the job `failed` and frees the concurrency-1 slot — it has no abort channel and cannot stop the runner. It is therefore **derived**, not hardcoded: `ASSEMBLY_TIMEOUT_MS = DEFAULT_TIMEOUT_MS + ASSEMBLY_NON_SOFFICE_BUDGET_MS` = 180 s + 120 s = **300 s** (`src/server/assembly/assemblyBudget.ts`, asserted in `assemblyBudget.test.ts`). Were the registry able to fire first, it would promote a queued job while the original soffice was still alive, breaking concurrency-1 — and two headless LibreOffice instances on the deployed 2 GB swapless box risk an OOM kill.

  **Measured baseline** (M3, real fixtures, real soffice, Luke series 2), superseding the spike's ~40 s figure: 14× copy + `prepareConstituentForAssembly` 2,672 ms; `sofficeAssemble` (14 inserts) 15,259 ms; `finalizeAssembledQuarter` 381 ms; 14× `mergeXml` unzip/zip 1,450 ms as a lower bound on `makeLessonFile`. Non-soffice work ≈ 9 s here, ≈ 21–27 s on the ~2.5–3.5× slower deployed box — hence the 120 s non-soffice budget.

- **Low-memory admission**: a new job is refused (same `429` contract as the queue-depth cap) when Linux `MemAvailable` is below a floor. Read from `/proc/meminfo`, not `os.freemem()` — on the deployed box `freemem` reported 303 MB while `MemAvailable` was 1.31 GB. Unreadable (macOS dev, no `/proc`) disables the guard.
